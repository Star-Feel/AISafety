...
def diffattack(
    ...
    if args.dataset_name == "imagenet_compatible":
        ...
        from dataset_caption import cifar_label as imagenet_label
    else:
    ...
    if args.dataset_name != 'cifar':
        ...
        test_image[:, :, ] -= (np.float32(0.4914), np.float32(0.4822), np.float32(0.4465))
        test_image[:, :, ] /= (np.float32(0.2023), np.float32(0.1994), np.float32(0.2010))
    ...
    for _, _ in enumerate(pbar):
        ...
        if args.dataset_name == 'cifar':
            mean = torch.as_tensor([0.4914, 0.4822, 0.4465], dtype=out_image.dtype, device=out_image.device)
            std = torch.as_tensor([0.2023, 0.1994, 0.2010], dtype=out_image.dtype, device=out_image.device)
        else:
        ...
        if args.dataset_name != "imagenet_compatible" and args.dataset_name != "cifar":
