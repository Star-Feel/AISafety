...
from third_party.pytorch_cifar.models import SimpleDLA, ResNet101, ResNet18
...
def model_selection(name):
    if name == "convnext":
        ...
        model = SimpleDLA()
        ckpt = torch.load('./third_party/pytorch_cifar/checkpoint/simpledla_ckpt.pth')['net']
        for key in list(ckpt.keys()):
            ckpt[key.replace('module.', '')] = ckpt.pop(key)
        model.load_state_dict(ckpt)
    elif name == 'resnet101':
        model = ResNet101()
        ckpt = torch.load('./third_party/pytorch_cifar/checkpoint/resnet101_ckpt.pth')['net']
        for key in list(ckpt.keys()):
            ckpt[key.replace('module.', '')] = ckpt.pop(key)
        model.load_state_dict(ckpt)
    elif name == 'resnet18':
        model = ResNet18()
        ckpt = torch.load('./third_party/pytorch_cifar/checkpoint/resnet18_ckpt.pth')['net']
        for key in list(ckpt.keys()):
            ckpt[key.replace('module.', '')] = ckpt.pop(key)
        model.load_state_dict(ckpt)
    else:
...
def model_transfer(clean_img, adv_img, label, res, save_path=r"C:\Users\PC\Desktop\output", fid_path=None, args=None):
    ...
    if args.dataset_name == "imagenet_compatible":
        ...
        models_transfer_name = ["simpledla", "resnet101"]
        nb_classes = 10
    else:
