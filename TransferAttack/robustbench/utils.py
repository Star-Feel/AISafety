...
def load_model_from_ckpt(model_name: str,
                         ckpt_path: Union[str, Path],
                         dataset: Union[str, BenchmarkDataset] = BenchmarkDataset.cifar_10,
                         threat_model: Union[str, ThreatModel] = ThreatModel.Linf,
                         norm: Optional[str] = None) -> nn.Module:
    """
    Load a model from a local checkpoint, following the same logic as load_model,
    but bypassing Google Drive download entirely.

    :param model_name: The name used in the model zoo.
    :param ckpt_path: Path to the local checkpoint (.pt, .pth, or .pth.tar file).
    :param dataset: The dataset on which the model is trained.
    :param threat_model: The threat model for which the model is trained.
    :param norm: Deprecated argument that can be used in place of threat_model.
    ...
    # 如果是 timm 模型
    if timm.is_model(timm_model_name):
        print(f"Loading timm model `{timm_model_name}` from checkpoint `{ckpt_path}`.")
    ...
    if model_name not in models:
        raise ValueError(f"Model `{model_name}` not found in local all_models dictionary.")

    model_info = models[model_name]
    model = model_info['model']()

    # 支持 ensemble 模型
    gdrive_ids = model_info.get('gdrive_id', None)
    is_ensemble = isinstance(gdrive_ids, list)

    if not is_ensemble:
        # 单模型
        checkpoint = torch.load(ckpt_path, map_location='cpu')
        if isinstance(checkpoint, dict) and 'last' in checkpoint:
        ...
        # ensemble 模型
        checkpoints = [torch.load(str(ckpt_path), map_location='cpu') for _ in range(len(gdrive_ids))]
        ...
        for i, checkpoint in enumerate(checkpoints):
            ...
            if model_name.startswith('Bai2023Improving'):
                ...
                if i == 0:
                    model.std_model = _safe_load_state_dict(model.std_model, model_name, state_dict, dataset_)
                elif i == 1:
                    if dataset_ == BenchmarkDataset.imagenet:
                        from timm.models.swin_transformer import checkpoint_filter_fn
                        state_dict = checkpoint_filter_fn(state_dict, model.rob_model.model)
                        state_dict = add_substr_to_state_dict(state_dict, 'model.')
                    model.rob_model = _safe_load_state_dict(model.rob_model, model_name, state_dict, dataset_)
                else:
                    raise ValueError('Unexpected checkpoint index for ensemble.')
            else:
                model.models[i] = _safe_load_state_dict(model.models[i], model_name, state_dict, dataset_)
                model.models[i].eval()
