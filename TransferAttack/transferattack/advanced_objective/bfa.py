...
class BFA(MIFGSM):
    ...
    def register_hook(self):
        ...
        if target_module is None:
            ...
            if fallback is None:
                available = ', '.join(dict(backbone.named_modules()).keys())
                raise ValueError(
                    f"Layer {self.layer_name} not found in backbone. Available modules: {available[:2000]}"
                )
            fallback_name, target_module = fallback
            warnings.warn(
                f"Layer {self.layer_name} not found; falling back to {fallback_name} for BFA feature hooks"
            )
            self.layer_name = fallback_name
