...
class Attack(object):
    ...
    def __init__(self, attack, model_name, epsilon, targeted, random_start, norm, loss, device=None):
        ...
        self.attack = attack
        preloaded_model = getattr(self, '_preloaded_model', None)
        if preloaded_model is not None:
            self.model = preloaded_model
            delattr(self, '_preloaded_model')
        else:
    ...
    def load_model(self, model_name):
        ...
        if isinstance(model_name, nn.Module):
            # Allow the caller to inject an already-loaded backbone (e.g., from the pipeline).
            return model_name
