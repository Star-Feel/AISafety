...
class ANA(MIFGSM):
    ...
    def __init__(self, model_name, epsilon=16 / 255, alpha=1.6 / 255, epoch=10, decay=1., num_ens=30,
                 ...
                 norm='linfty', loss='crossentropy', device=None, attack='P2FA', eta=28.0, gamma=2.0, checkpoint_path='./checkpoints/', **kwargs):
    ...
    def forward(self, data, label):
        ...
        with torch.no_grad():
            _, feat_orig_flat, feat_mask_aligned_ref = self.net(x, x)
            ref_pre = feat_orig_flat.detach()
            ref_post = feat_mask_aligned_ref.detach()
        momentum = 0
        for step in range(self.epoch):
            x_adv = (x + delta).requires_grad_(True)
            logits_adv, feat_adv_pre, feat_adv_post_aligned = self.net(x_adv, x_adv)
