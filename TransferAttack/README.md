...
### CIFAR10_clean Pipeline

`pipeline.py` unifies CIFAR10 fine-tuning, latest transfer attacks, and evaluation on `datasets/CIFAR10_clean` (labels/images). Key details:

- Fine-tunes every `--surrogates` backbone on torchvision CIFAR10 (dataloader is aligned with `../GGS/pipeline.py`). Use `--skip_finetune` after checkpoints live in `checkpoints/cifar10`, or `--force_finetune` to retrain.
- Runs the newest README attacks by default (`mef`, `anda`, `mumodig`, `gaa`, `foolmix`, `bsr`, `ops`, `bfa`, `p2fa`, `ana`, `ll2s`, `smer`) with optional ensembles (`--attack_ensembles resnet50+vit_base_patch16_224`).
- Certain attacks (currently `bfa`, `p2fa`) only support single-model surrogates; the pipeline auto-splits multi-model ensembles and logs a warning when this happens.
- Chain arbitrary attacks sequentially via the `attackA->attackB` syntax (e.g., `--attacks "mef->p2fa->ana"`). Remember to quote the entire expression (or escape the `>` symbols) so your shell does not treat them as redirections. The pipeline instantiates a single serial wrapper, reuses the same surrogate weights, and enforces single-model mode automatically if any stage (such as `bfa`/`p2fa`) requires it. Outputs land under `attackout/<sanitized-attack>/<ensemble>/`, so `mef->p2fa` becomes `attackout/mef__p2fa/...`.
- Saves adversarial PNGs to `attackout/<attack>/<ensemble>/` (optionally resized with `--save_size`) and logs per-batch SSIM.
- Evaluates each specified model with the metric `100 * ARC * SSIM` (ARC is ASR for untargeted mode and accuracy for targeted mode). Summaries are appended to `log/pipeline_results.json` with the raw log stored in `log/`.

#### Pipeline workflow at a glance

1. **Finetune / reuse checkpoints** – every surrogate listed in `--surrogates` is (re)trained unless `--skip_finetune` finds an existing file. Models are stored at `checkpoints/cifar10/<model>_cifar10.pth`.
2. **Attack generation** – for each attack in `--attacks` and each combination produced from `--attack_ensembles`, the pipeline builds the surrogate(s), optionally expands serial chains, and saves adversarial PNGs under `attackout/`.
3. **Evaluation + logging** – all `--eval_models` (defaults to the surrogates) score the generated images. Metrics and summaries are appended to `log/pipeline_results.json`, while verbose logs land in `log/pipeline_<timestamp>.log`.

#### Common scenarios and commands

| Scenario | Command / Notes |
| --- | --- |
| Train everything once, full default sweep | `python pipeline.py --clean_dir datasets/CIFAR10_clean --cifar_root datasets/CIFAR10 --surrogates resnet50 vit_base_patch16_224` |
| Reuse existing checkpoints only | Add `--skip_finetune` (set `--force_finetune` to rebuild specific models). |
| Limit to a subset of attacks | `--attacks mef ops bsr` (space-separated list of plain names). |
| Serial combo that mixes attack families | `--attacks "mef->p2fa->ops" --surrogates resnet50 --attack_ensembles resnet50`. Quoting is mandatory on most shells; serial names are sanitized to folder-friendly slugs such as `attackout/mef__p2fa__ops/resnet50`. |
| Multi-model ensemble run | Provide both `--surrogates` and `--attack_ensembles resnet50+vit_base_patch16_224`. If a serial chain contains a single-model-only stage (e.g., `p2fa`), the pipeline automatically splits ensembles and prints a warning instead of failing. |
| Targeted evaluation | Append `--targeted --target_file datasets/CIFAR10_clean/targets.txt`. The `target_file` format mirrors `label.txt`. |
| Quick smoke test | Use `--max_batches 2` (or similar) to shorten runtime when debugging heavy attacks such as MEF or serial mixes. |
| Evaluation-only pass on stored adversaries | Point `--clean_dir` to the dataset, keep `--attack_out` pointing at the folder containing prior outputs, and run with `--skip_finetune` plus `--attacks <name>` to regenerate metrics without recomputing perturbations. |

Example (untargeted, CIFAR10 dirs already prepared):

```bash
python pipeline.py \
  --clean_dir datasets/CIFAR10_clean \
  --cifar_root datasets/CIFAR10 \
  --surrogates resnet50 vit_base_patch16_224 \
  --batchsize 32 --epochs 20
```

To cascade attacks sequentially, pass quoted specs such as:

```bash
python pipeline.py \
  --attacks "mef->p2fa->ops" \
  --surrogates resnet50 \
  --attack_ensembles resnet50
```
